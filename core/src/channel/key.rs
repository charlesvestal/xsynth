    ...
    pub fn send_event(
        ...
        match event {
            ...
            KeyNoteEvent::Off => {
                ...
                // MOVE FORK / 2026-05-17: removed render-thread file I/O
                // for the noteoff-no-op diagnostic. With damper=true (e.g.
                // sustain pedal held) EVERY noteoff is a no-op; the
                // resulting fopen/fprintf/fclose per noteoff was adding
                // hundreds of µs to render time on Move's SD card and
                // contributing to host-side SPI frame overruns. The
                // damper-held state is still observable via the per-block
                // perf log breakdown.
            }
            KeyNoteEvent::AllOff => {
